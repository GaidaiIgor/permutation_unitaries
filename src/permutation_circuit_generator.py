...
from src.utilities.quantum import get_average_neighbors, find_min_control_set_2, get_cx_cost_cx
...
class PermutationCircuitGeneratorPairwise(PermutationCircuitGenerator):
...
class PermutationCircuitGeneratorCluster(PermutationCircuitGenerator):
    """ Generates permutation circuit that takes into account cluster permutations. """
    ...
    def get_permutation_circuit(self, permutation: dict[str, str]) -> QuantumCircuit:
        ...
        def find_best_swap(all_bases_old: ndarray, difference: ndarray) -> Swap:
            """ Finds a swap that maximizes (attempts to) the number of errors fixed per number of CX gates required. """
            @dataclass
            class Group:
                coords: (int, ...)
                row_inds: list[int]
            ...
            def form_groups(all_bases_old: ndarray, difference: ndarray, selected_cols: list[int]) -> dict[(int, ...), Group]:
                """ Forms groups of amplitudes that belong to the same node in a swap graph. """
                groups = {}
            ...
            def build_swap_graph(groups: dict[(int, ...), Group]) -> DiGraph:
                """ Builds a swap graph from groups of amplitudes. """
                def update_group_location(group: Group, all_coords: list[(int, ...)], graph: DiGraph):
                    """ Adds node and edges corresponding to a given group to the swap graph. """
                    def find_best_swap_gate(all_coords: list[(int, ...)], distinguish_ind: int, candidate_target_inds: ndarray) -> SwapGate:
                        """ Finds minimally necessary set of controls to select specified index. """
        ...
        def implement_swap(swap: Swap, num_ancillas: int, qc: QuantumCircuit, all_bases_old: ndarray, difference: ndarray):
            """ Implements given swap in a quantum circuit. """
            ...
            if len(swap.gate.control_inds) == 0:
                ...
                cx_target_inds = selected_cols[swap.gate.conjugated_target_inds[swap.gate.conjugated_target_inds != swap.gate.target_ind]]
